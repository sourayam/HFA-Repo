/*                                      Kamloops Customer View
Data Source: ActiveNet User interface - "Membership by Customer" 
-recall schedule: "Membership for SQL"
-filters include all (retention and non-retention eligible memberships) - Total memberships from census report includes memberships and community
-numbers to align with census reports

Additional considerations: Program pass is the same as community in YGV, General, platinum, and gold memberships all have the same price except platinum and gold receive towels and lockers
summer teen membership is at no cost
Purpose: Use in powerBI to create HFA new customer dashboard
*/

--Drop view kam_CustomerPBI12182023
...
Query3 as 
(select * from Query2 
where Category in ('Membership', 'Community')),
...
when PackageName like '%Program Pass%' then 'Program Pass'
